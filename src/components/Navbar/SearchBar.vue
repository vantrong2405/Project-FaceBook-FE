...
<template>
  ...
  >
    ...
    <div
      class="h-9 flex items-center justify-center rounded-full cursor-pointer hover:bg-myGray-200 text-myGray-800 transition-opacity duration-500"
      :class="!isFocus ? 'absolute transform -translate-x-96 opacity-0' : 'opacity-100'"
    >
      <svg-left class="w-5" />
    </div>
    <div
      ...
      @click="isFocus = true"
      :class="isFocus ? 'flex-1 px-4 cursor-text' : ''"
    >
      <svg-search
        class="w-5 text-myGray-500"
        fill="currentColor"
        :class="isFocus ? 'absolute transform -translate-x-96 opacity-0' : 'opacity-100'"
      />
      <input
        class="w-full h-full bg-transparent text-sm px-2 border-none outline-none text-myGray-800 xl:block focus:outline-none focus:ring-transparent"
