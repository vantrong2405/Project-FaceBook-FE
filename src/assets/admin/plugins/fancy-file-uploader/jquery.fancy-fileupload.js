...
;(function ($) {
  ...
  var GetDisplayFilesize = function (numbytes, adjustprecision, units) {
    ...
    return (
      (pos == 0 || (adjustprecision && result >= 99.995) ? result.toFixed(0) : result.toFixed(2)) +
      " " +
      abbreviations[pos]
    )
  }
  ...
  $.fn.FancyFileUpload = function (options) {
    ...
    var AddFile = function (uploads, e, data) {
      ...
      var StartUpload = function (e) {
        ...
        inforow
          ...
          .text(data.ff_info.displayfilesize + " | " + Translate("Starting upload..."))
      ...
      if (settings.maxfilesize > -1 && data.files[0].size > settings.maxfilesize)
        data.ff_info.errors.push(
          FormatStr(
            Translate("File is too large.  Maximum file size is {0}."),
            GetDisplayFilesize(settings.maxfilesize, settings.adjustprecision, settings.displayunits)
          )
        )
      ...
      inforow
        ...
        .text(
          data.ff_info.displayfilesize +
            (hasimage && settings.edit && !data.ff_info.errors.length ? " | ." + fileext : "")
        )

      // Errors.
      if (data.ff_info.errors.length)
        inforow.find(".ff_fileupload_errors").html(data.ff_info.errors.join("<br>")).removeClass("ff_fileupload_hidden")
    ...
    var UploadProgress = function (e, data) {
      ...
      data.ff_info.fileinfo.text(
        FormatStr(
          Translate("{0} of {1} | {2}%"),
          GetDisplayFilesize(data.loaded, settings.adjustprecision, settings.displayunits),
          data.ff_info.displayfilesize,
          progress.toFixed(0)
        )
      )
    ...
    var UploadDone = function (e, data) {
      ...
      if (data.ff_info.removewidget) {
        ...
        data.ff_info.inforow
          .find(".ff_fileupload_fileinfo")
          .text(data.ff_info.displayfilesize + " | " + Translate("Upload completed"))
    ...
    return this.each(function () {
        ...
        if (!settings.accept) {
          ...
          for (var x = 0; x < accept.length; x++) {
            ...
            settings.accept.push(
              opt.indexOf("/") < 0 && opt.lastIndexOf(".") > -1 ? opt.substring(opt.lastIndexOf(".") + 1) : opt
            )
          }
        }
      }
      ...
      var dropzone = $("<button>")
        ...
        .attr("type", "button")
        .attr("aria-label", Translate("Browse, drag-and-drop, or paste files to upload"))
