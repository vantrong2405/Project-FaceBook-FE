<template>
  <div
    class="relative -z-30 flex h-14 w-72 items-center gap-1 py-2 sm:gap-2"
  ...
  >
    ...
    <div
      ...
      @click="isFocus = true"
      :class="isFocus ? 'flex-1 cursor-text px-4' : ''"
    >
      <svg-search
      ...
      <input
        ...
        @blur="isFocus = false"
        v-model="textSearch"
        @input="handleChangeIsShowAll()"
        onchange="searchInfoUser()"
        v-on:keyup.enter="this.$router.push(`/search/${this.textSearch}`)"
      />
    </div>
    <ul
      class="absolute left-[5px] top-[57px] z-50 w-[105%] cursor-pointer rounded-lg bg-white py-[5px] shadow-xl"
      v-if="textSearch"
    >
